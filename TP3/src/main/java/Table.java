...
public class Table {
    private final int height = 112;            // cm
    private final int width = 224;           // cm
    private final double epsilonMax = 0.03;
    private double ballDiameter = 5.7;     // cm
    ...
    public Table(final double initialBallYPos, final Double initialBallXSpeed, final List<Pair<Double, Double>> ballsEpsilon) {
        int ballIndex = 0;
        balls.put(ballIndex, new Ball(ballIndex++, ballDiameter, false, new Pair<>(56.0, initialBallYPos), new Pair<>(0.0, initialBallXSpeed)));

        double xyEpsilon = 2 * epsilonMax + ballDiameter;
        // Triangle balls
        for (int i=0 ; i < 5 ; i++) {
            ...
            for (int j=0 ; j <= i ; j++) {
                ...
                balls.put(ballIndex, new Ball(ballIndex++, ballDiameter, false, new Pair<>(xPos + ballEpsilon.getX(), initialYPos + j * xyEpsilon + ballEpsilon.getY())));
            }
        }
        ...
        for (double xMult=0.0 ; xMult < 1.1 ; xMult += 0.5) {
            for(double yMult=0.0 ; yMult < 1.1 ; yMult += 0.5) {
                if (!(xMult > 0.4 && xMult < 0.6) || !(yMult > 0.4 && yMult < 0.6)) {
                    balls.put(ballIndex, new Ball(ballIndex++, ballDiameter, true, new Pair<>(xMult * width, yMult * height)));
                }
            }
        }
